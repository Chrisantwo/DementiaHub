...
export function useCallSocket(sessionId: string | null | undefined): CallSocketState {
  ...
  useEffect(() => {
    ...
    // withCredentials: true ensures the session cookie is sent so the server
    // can authenticate this connection (same-origin already sends cookies,
    // but this makes the intent explicit and works if the origin diverges).
