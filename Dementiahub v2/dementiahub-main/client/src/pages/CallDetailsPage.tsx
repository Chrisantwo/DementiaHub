...
export default function CallDetailsPage() {
  ...
  const activeConversation = supportCaseQuery.data?.activeConversation;
  const chatMessages = supportCaseQuery.data?.chatMessages ?? [];
  ...
  function handleReplyKeyDown(event: React.KeyboardEvent<HTMLTextAreaElement>) {
    if ((event.metaKey || event.ctrlKey) && event.key === "Enter") {
      ...
      if (staffReply.trim() && !sendStaffCallChatReply.isPending) {
        sendStaffCallChatReply.mutate({ sessionId, content: staffReply });
      }
    }
  }
  ...
  const sendStaffCallChatReply = trpc.admin.sendStaffCallChatReply.useMutation({
    onSuccess: async () => {
      toast.success("Human chat reply sent");
      setStaffReply("");
      if (!sessionId) return;
      await Promise.all([
        utils.calls.getCallDetails.invalidate({ sessionId }),
        utils.admin.staffCallCase.invalidate({ sessionId }),
        utils.admin.staffDashboard.invalidate(),
        utils.ai.getMyConversation.invalidate(),
      ]);
    },
    onError: (error) => toast.error("Could not send human reply", { description: error.message }),
  });
              ...
              <Card>
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between gap-3">
                    <CardTitle className="text-sm font-semibold flex items-center gap-2">
                      ...
                      Reply to caregiver via chat
                      {session.humanTakeover ? (
                        <Badge className="bg-sky-100 text-sky-800 hover:bg-sky-100 text-[11px]">Human owned</Badge>
                      ) : null}
                    </CardTitle>
                    {activeConversation ? (
                      <Button
                        ...
                        onClick={() => setLocation(`/history/chat/${activeConversation.id}`)}
                      >
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed mt-1">
                    Your reply goes to the caregiver's portal chat as human staff. AI responses are paused while the thread is human-owned.
                  </p>
                </CardHeader>
                <CardContent className="space-y-3 pt-0">
                  {chatMessages.length > 0 ? (
                    <div
                      ref={chatScrollRef}
                      className="max-h-[360px] space-y-3 overflow-y-auto rounded-2xl border border-[#ddd3c4] bg-white/65 p-4"
                    >
                      {chatMessages.map((message: any) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className={`max-w-[82%] space-y-1 ${isCaregiver ? "items-end" : "items-start"} flex flex-col`}>
                    ...
                    <div className="rounded-2xl border border-dashed border-[#ddd3c4] bg-white/55 px-4 py-6 text-center text-sm text-muted-foreground">
                      No portal chat thread yet. Sending a reply will create one for this caregiver.
                    </div>
                  )}
                  ...
                  <div className="relative">
                    <Textarea
                      ...
                      rows={3}
                      placeholder="Type a reply to the caregiver as human staff..."
                      className="resize-none pr-4 pb-7"
                    />
                    <p className="absolute bottom-2 right-3 text-[11px] text-muted-foreground pointer-events-none">
                      {staffReply.length > 0 ? `${staffReply.length} · ` : ""}⌘↵ to send
                    </p>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Button
                      className="ml-auto rounded-full bg-[#1d4e4b] hover:bg-[#0f2e2c]"
                      disabled={!staffReply.trim() || sendStaffCallChatReply.isPending}
                      onClick={() => sendStaffCallChatReply.mutate({ sessionId, content: staffReply })}
                    >
                      {sendStaffCallChatReply.isPending ? (
