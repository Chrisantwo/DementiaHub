...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<User> {
    ...
    // Only write lastSignedIn when it's been more than 5 minutes since the
    // last update — every authenticated tRPC call would otherwise hammer the DB.
    const THROTTLE_MS = 5 * 60 * 1000;
    if (Date.now() - user.lastSignedIn.getTime() > THROTTLE_MS) {
