...
export async function updateCallSession(
  ...
  if (Object.values(updates).every(v => v === undefined)) return;
  const db = await getDb();
  if (!db) return;
...
export async function getSupportCallCase(sessionId: string) {
  ...
  const [callbacks, transcriptChunks, assignedStaff, activeConversation] = await Promise.all([
    getCallbackAttemptsForCase({
    ...
    rows[0].session.assignedStaffUserId
      ...
      : Promise.resolve(undefined),
    getActiveAIChatConversationByUserId(rows[0].session.portalUserId),
  ]);
  const chatMessages = activeConversation
    ? await getAIChatMessagesByConversationId(activeConversation.id, 50)
    : [];
