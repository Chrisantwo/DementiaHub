...
describe("verifyElevenLabsWebhookSecret", () => {
  it("returns true when the actual configured secret is provided", () => {
    // Hardcoded fallbacks were removed for security; the runtime secret is
    // either the env-supplied value or a random per-process value in dev.
    expect(verifyElevenLabsWebhookSecret("post_call", config.elevenLabsPostCallWebhookSecret)).toBe(true);
    expect(verifyElevenLabsWebhookSecret("consent", config.elevenLabsConsentWebhookSecret)).toBe(true);
  });

  it("rejects the legacy hardcoded fallback secret (security regression)", () => {
    const legacyHardcodedFallback = "dementiahub-webhook-secret-2026";
    expect(verifyElevenLabsWebhookSecret("post_call", legacyHardcodedFallback)).toBe(false);
    expect(verifyElevenLabsWebhookSecret("consent", legacyHardcodedFallback)).toBe(false);
  });
