...
async function ensureStaffChatConversationForUser(portalUserId: number) {
  ...
  if (!conversationId) {
    throw new TRPCError({
      ...
      message: "Could not create caregiver chat thread",
    });
  }
  ...
  if (!created) {
    throw new TRPCError({
      code: "INTERNAL_SERVER_ERROR",
      message: "Created caregiver chat thread could not be loaded",
    });
  }
...
export const adminRouter = router({
  ...
  sendStaffCallChatReply: staffProcedure
    .input(
      z.object({
        sessionId: z.string().min(1),
        content: z.string().min(1).max(8000),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const now = new Date();
      const conversation = await ensureStaffChatConversationForUser(session.portalUserId);
      ...
      await updateAIChatConversation(conversation.id, {
        ...
        caseStatus: "pending_caregiver",
        casePriority:
          session.casePriority === "urgent" || session.casePriority === "high"
            ? session.casePriority
            : undefined,
        callbackRequested: session.callbackRequested || undefined,
        escalationTriggered: session.escalationTriggered || undefined,
        safetyResult: session.safetyResult ?? undefined,
        safetyFlagType: session.safetyFlagType ?? undefined,
        topicClassified: session.topicClassified ?? undefined,
        resolutionType: session.resolutionType ?? undefined,
      });
      await updateCallSession(input.sessionId, {
        humanTakeover: true,
        humanTakeoverAt: session.humanTakeover ? session.humanTakeoverAt : now,
        assignedStaffUserId: ctx.user.id,
        lastStaffResponseAt: now,
        caseStatus: "pending_caregiver",
      });
