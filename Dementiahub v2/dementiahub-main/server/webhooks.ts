...
// .passthrough() keeps unknown fields so downstream services receive the full
// payload. Validation only ensures the shape we depend on is sound.

const postCallPayloadSchema = z
  .object({
    conversation_id: z.string().optional(),
    elevenlabs: z
      .object({ conversation_id: z.string().optional() })
      .passthrough()
      .optional(),
  })
  .passthrough();

const dynamicVariablesSchema = z
  .object({
    ghl_contact_id: z.string().optional(),
    wibiz_contact_id: z.string().optional(),
  })
  .passthrough();
...
webhookRouter.post("/elevenlabs/post-call", async (req, res) => {
  ...
  const rawPayload = unwrapWebhookPayload(req.body);
  const parsed = postCallPayloadSchema.safeParse(rawPayload);
  if (!parsed.success) {
    console.warn("[Webhook] post-call payload failed validation:", parsed.error.issues);
  ...
  try {
    // Pass rawPayload so processPostCallWebhook receives the full untyped payload
    // as it expects. parsed.data is used only for the typed fields read above.
    const resolvedId = await processPostCallWebhook(rawPayload);
...
webhookRouter.post("/elevenlabs/consent", async (req, res) => {
  ...
  const rawPayload = unwrapWebhookPayload(req.body);
  const parsed = consentPayloadSchema.safeParse(rawPayload);
  if (!parsed.success) {
    console.warn("[Webhook] consent payload failed validation:", parsed.error.issues);
    return res.status(400).json({ error: "Invalid payload structure" });
  }
