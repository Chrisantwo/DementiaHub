...
function normalizeSafetyResult(value: string | undefined) {
  const normalized = value?.trim().toUpperCase();
  return normalized === "SAFE" || normalized === "CAUTION" || normalized === "UNSAFE"
    ? normalized
    : "";
}

function shouldPreferLocalTriage(params: {
  externalSafetyResult?: string;
  localSafetyResult: string;
  outcome: NonNullable<PostCallPayload["call_outcome"]>;
}) {
  const externalSafety = normalizeSafetyResult(params.externalSafetyResult);
  const localSafety = normalizeSafetyResult(params.localSafetyResult);

  if (!externalSafety) return true;
  if (!localSafety) return false;
  if (SAFETY_SEVERITY[localSafety] > SAFETY_SEVERITY[externalSafety]) return true;

  return (
    !params.outcome.topic_classified ||
    !params.outcome.resolution_type ||
    params.outcome.callback_requested === undefined ||
    params.outcome.escalation_triggered === undefined
  );
}
...
export function deriveOutcome(params: {
  ...
  if (params.transcriptEntries.length === 0) {
    return null;
  }
  ...
  const preferLocalTriage = shouldPreferLocalTriage({
    externalSafetyResult: params.outcome.safety_result,
    localSafetyResult: triage.safetyResult,
    outcome: params.outcome,
  });

  if (!preferLocalTriage) {
    return {
      safety_flag_type: params.outcome.safety_flag_type || triage.safetyFlagType,
      call_summary: params.outcome.call_summary || triage.conversationSummary || params.fallbackSummary,
    };
  }
...
export async function processPostCallWebhook(rawPayload: PostCallPayload): Promise<string> {
  ...
  try {
    ...
    try {
      const createdTranscript = await CallTranscript.create(transcriptDetails);
      const newTranscriptId = String(createdTranscript._id);
      console.log(
        `[PostCallSync] transcript details persisted to MongoDB conversation=${conversationId} _id=${newTranscriptId}`
      );

      // Append new transcript _id to the GHL contact's "Transcript IDs (MongoDB)" custom field.
      // The field is treated as a CSV string for debugging purposes.
      try {
        // Fetch the contact's current custom field value via GHL REST API.
        ...
        if (contactRes.ok) {
          ...
          const fields: Array<{ id: string; key?: string; field_key?: string; value: string }> =
            contactData?.contact?.customFields ?? [];
          const match = fields.find(
            (f) =>
              f.key === "transcript_id_mongodb" ||
              f.field_key === "transcript_id_mongodb" ||
              f.key === "transcript_id_mongodb" ||
              f.field_key === "transcript_id_mongodb"
          );
          existingIds = match?.value?.trim() ?? "";
        }

        const updatedIds = existingIds ? `${existingIds},${newTranscriptId}` : newTranscriptId;

        await updateContact(config.ghlApiKey!, ghlContactId, {
          customFields: [
            { key: "transcript_ids_mongodb", value: updatedIds },
          ],
        });

        console.log(
          `[PostCallSync] GHL contact transcript IDs updated contact=${ghlContactId} ids=${updatedIds}`
        );
      } catch (fieldErr: any) {
        console.error(
          `[PostCallSync] failed to update transcript IDs on GHL contact contact=${ghlContactId} conversation=${conversationId}`,
          fieldErr
        );
      }
    } catch (err: any) {
      console.error(
        `[PostCallSync] failed to persist transcript details conversation=${conversationId}`,
        err
      );
    }  
